...
export default function TaxCalculator() {
  ...
  // 🛠️ React Hook Form সেটআপ Zod স্কিমার সাথে
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  // 🔄 প্রোডাকশন-গ্রেড পোলিং ইঞ্জিন (Axios সার্ভিস দিয়ে আপডেট করা)
  const pollTaskStatus = async (taskId: string, currentRetry = 0) => {
    const MAX_RETRIES = 15; // সর্বোচ্চ ১৫ বার চেক করবে
    const POLLING_INTERVAL = Number(process.env.NEXT_PUBLIC_POLLING_INTERVAL) || 2000;
    ...
    try {
      // Axios সার্ভিস দিয়ে স্ট্যাটাস চেক
      ...
      if (data.status === 'COMPLETED') {
        ...
        // PENDING বা PROCESSING থাকলে আবার পোল করবে
        setTimeout(() => pollTaskStatus(taskId, currentRetry + 1), POLLING_INTERVAL);
      }
    } catch (err: any) {
      console.error('Polling error:', err);
      // নেটওয়ার্ক ড্রপ করলেও রিট্রাই সচল রাখা
      setTimeout(() => pollTaskStatus(taskId, currentRetry + 1), POLLING_INTERVAL);
    }
  };
  ...
  const onSubmit = async (formData: TaxFormInputs) => {
    try {
      ...
      // আপনার এপিআই রাউটের রিকোয়েস্ট বডি ফরম্যাট অনুযায়ী ডেটা পাঠানো হলো
